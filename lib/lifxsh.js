  ...
  list: function () {
    ...
    onlineOffline.forEach(light => {
      ...
      statePromise.then(state => {
        let lightDetails = {};
        lightDetails.id = light.id;
        lightDetails.ip = light.address;
        lightDetails.label = light.label;
        lightDetails.brightness = state.color.brightness + '%';
        lightDetails.status = state.power === 1 ? 'on' : 'off';
        detailsList.push(lightDetails);
      })
    });

    Promise.all(statePromises).then(() => {
      log.table(detailsList);
    });
  },
